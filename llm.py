...
- НЕ ПРИБЛИЖАЙ ПЕРИОД. Если период нельзя выразить ТОЧНО поддержанными фильтрами
  ...
  (уйдёт в L2, который умеет произвольные диапазоны дат).
- НЕ ПРИБЛИЖАЙ АГРЕГАЦИЮ. У каждой метрики агрегация фиксирована (Total/Net/оборот =
  ...
  {{"metric": null, "reason": "нестандартная агрегация"}} (уйдёт в L2).
  НО «средний депозит/средний чек» — это готовая метрика Average Deposit, бери её.
...
- ПЛАН/ФАКТ, выполнение плана, сравнение с таргетом («выполнили ли план»,
  ...
  верни {{"metric": null, "reason": "план vs факт"}} (уйдёт в L2). НЕ пытайся
...
def _extract_sql(text: str) -> str:
    """Достаём чистый запрос из ответа модели (снимаем markdown-обёртку).
