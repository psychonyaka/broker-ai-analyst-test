...
`python eval.py` — золотой тест-сет по 6 группам. На **gpt-4o**:
...
| longtail_answered (L2) | оконки / доли / план-vs-факт | 2/2 |
| chitchat_rejected (L2) | не выдаёт SQL на болтовню/оскорбления | 3/3 |
| definitions (L0) | ответ-определение из слоя, без SQL | 3/3 |

На **fallback без ключа** — plan ~90%, exec 100%, reject 100% (промахи только на
англоязычных вопросах: fallback заточен под русские синонимы, настоящая LLM их
берёт — наглядно, зачем LLM поверх слоя). Ретрай поднимает exec на слабых моделях
(gpt-4o-mini: 96% → 100%).
