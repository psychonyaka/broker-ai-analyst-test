...
DATA = "#E8B400"        # золотой для данных на светлом (читается на белом)
INK = "#1B1D24"         # основной текст
MUTED = "#6B7280"       # приглушённый текст
GRID = "#E5E7EB"        # линии сетки
...
def _svg_line_chart(df: pd.DataFrame, width: int = 640, height: int = 240) -> str:
...
def _svg_heatmap(df: pd.DataFrame, cell: int = 46) -> str:
...
def auto_viz(df: pd.DataFrame):
    ...
    if ncols == 2 and pd.api.types.is_numeric_dtype(df.iloc[:, 1]):
        ...
        if _is_temporal(df[dim], str(dim)):
            return _svg_line_chart(df), 260
        return _svg_bar_chart(df), min(n, 15) * 34 + 24
    # два разреза + мера -> хитмап
