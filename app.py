...
from report import auto_viz  # та же визуализация, что и в HTML-отчете (KPI/линия/бары/хитмап)
...
with st.sidebar:
    ...
    provider = st.selectbox(
        ...
        help="auto: возьмет ключ из env; fallback работает без ключа")
...
def render(ans):
    ...
    # авто-график: инлайновый SVG (тот же, что в HTML-отчете). Не зависит от
    # Altair/Vega, поэтому рисуется даже там, где у Altair проблемы с сертификатами.
    viz, h = auto_viz(df, dark=True)  # Streamlit темный -> светлый текст на графиках
    if viz:
